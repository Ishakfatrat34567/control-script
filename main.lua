...
local BOT_PROTOCOL_PREFIX = "[bot-sync:v3]"
local BOT_PROTOCOL_TAG = "control-script"
local BOT_HEARTBEAT_INTERVAL = 15
local BOT_STALE_TIMEOUT = 50

local fellowBots = {}
local botSeenAt = {}
local botSessionByPlayer = {}
local botNumber = 1
local totalBots = 1

local rng = Random.new()
local botSessionId = tostring(localPlayer.UserId)
	.. "-"
	.. tostring(math.floor(os.clock() * 1000))
	.. "-"
	.. tostring(rng:NextInteger(1000, 9999))
...
local function buildBotProtocolMessage(kind)
	return table.concat({
		BOT_PROTOCOL_PREFIX,
		"kind=" .. tostring(kind or "heartbeat"),
		"uid=" .. tostring(localPlayer.UserId),
		"session=" .. botSessionId,
		"tag=" .. BOT_PROTOCOL_TAG,
	}, "|")
end

local function parseBotProtocolMessage(message)
	local rawMessage = (message or ""):gsub("^%s+", ""):gsub("%s+$", "")
	if rawMessage == "" then
		return nil
	end

	local parts = {}
	for part in string.gmatch(rawMessage, "[^|]+") do
		table.insert(parts, part)
	end

	if parts[1] ~= BOT_PROTOCOL_PREFIX then
		return nil
	end
	...
	local userId = tonumber(payload.uid or "")
	if payload.tag ~= BOT_PROTOCOL_TAG or not userId or userId <= 0 then
		return nil
	end
...
local function cleanupKnownBots()
	local now = os.clock()
	for player in pairs(fellowBots) do
		local lastSeen = botSeenAt[player]
		if not player or player.Parent ~= Players or not lastSeen or (now - lastSeen) > BOT_STALE_TIMEOUT then
			fellowBots[player] = nil
			botSeenAt[player] = nil
			botSessionByPlayer[player] = nil
		end
	end
end
...
local function registerFellowBot(player, botInfo)
	if not player or player == localPlayer or not botInfo or botInfo.uid ~= player.UserId then
		return
	end

	fellowBots[player] = true
	botSeenAt[player] = os.clock()
	botSessionByPlayer[player] = botInfo.session or tostring(player.UserId)
	recalculateBotNumbering()
end

local function maybeRegisterBotFromMessage(player, message)
	local botInfo = parseBotProtocolMessage(message)
	if botInfo then
		registerFellowBot(player, botInfo)
		if botInfo.kind == "hello" then
			task.defer(function()
				sendChatMessage(buildBotProtocolMessage("ack"))
			end)
		end
	end
end
...
local function sendCommandList()
	local pages = {
		"[check 1/3] /follow /stack [name] /side line /orbit /line /fly /fireworks /stop /funfact /swarm [name]",
		"[check 2/3] /cross /triangle /square /stairs /freeze /vline /zigzag /diamond /spiral /shield",
		"[check 3/3] /gate /crown /arrow /bridge /dice /reset /say <msg> /calc <a+b|a/b> /auth <name> /unauth <name>",
	}
...
local function getGateOffset(companionIndex)
	local side = (companionIndex % 2 == 0) and 1 or -1
	local layer = math.floor((companionIndex - 1) / 2)
	local x = side * (FOLLOW_SPACING * 1.5)
	local y = SHOULDER_HEIGHT_OFFSET + (layer * FOLLOW_SPACING)
	return Vector3.new(x, y, 0)
end

local function getCrownOffset(companionIndex, companionCount)
	if companionCount <= 1 then
		return Vector3.new(0, SHOULDER_HEIGHT_OFFSET + FOLLOW_SPACING, -FOLLOW_SPACING)
	end

	local t = (companionIndex - 1) / math.max(companionCount - 1, 1)
	local angle = math.pi - (math.pi * t)
	local radius = FOLLOW_SPACING * 1.8
	local x = math.cos(angle) * radius
	local y = SHOULDER_HEIGHT_OFFSET + FOLLOW_SPACING + (math.sin(angle) * (FOLLOW_SPACING * 0.9))
	return Vector3.new(x, y, -FOLLOW_SPACING)
end

local function getArrowOffset(companionIndex)
	local row = math.floor((companionIndex - 1) / 2)
	local side = ((companionIndex % 2) == 0) and 1 or -1
	if companionIndex == 1 then
		return Vector3.new(0, SHOULDER_HEIGHT_OFFSET, -FOLLOW_SPACING)
	end

	local x = side * row * (FOLLOW_SPACING * 0.9)
	local z = -FOLLOW_SPACING - (row * FOLLOW_SPACING)
	return Vector3.new(x, SHOULDER_HEIGHT_OFFSET, z)
end

local function getBridgeOffset(companionIndex, companionCount)
	if companionCount <= 1 then
		return Vector3.new(0, SHOULDER_HEIGHT_OFFSET + FOLLOW_SPACING, 0)
	end

	local t = (companionIndex - 1) / math.max(companionCount - 1, 1)
	local x = (t - 0.5) * FOLLOW_SPACING * math.max(companionCount - 1, 1)
	local y = SHOULDER_HEIGHT_OFFSET + FOLLOW_SPACING + ((1 - math.abs((t * 2) - 1)) * FOLLOW_SPACING * 1.2)
	return Vector3.new(x, y, 0)
end

local function getDiceOffset(companionIndex)
	local zeroIndex = companionIndex - 1
	local column = zeroIndex % 2
	local row = math.floor(zeroIndex / 2)
	local x = (column == 0) and (-FOLLOW_SPACING * 0.8) or (FOLLOW_SPACING * 0.8)
	local y = SHOULDER_HEIGHT_OFFSET + ((1 - row) * FOLLOW_SPACING)
	return Vector3.new(x, y, 0)
end

local function getBotFormationIndex()
	local companions = getBotRoster()
...
local function startMotion(mode, targetPlayer, optionalStackTarget)
	...
	motionConnection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		local companionIndex, companionCount = getBotFormationIndex()
		...
		elseif motionMode == "gate" then
			local gateOffset = getGateOffset(companionIndex)
			moveNearTarget(targetRoot, CFrame.new(gateOffset), 0.28)
		elseif motionMode == "crown" then
			local crownOffset = getCrownOffset(companionIndex, companionCount)
			moveNearTarget(targetRoot, CFrame.new(crownOffset), 0.28)
		elseif motionMode == "arrow" then
			local arrowOffset = getArrowOffset(companionIndex)
			moveNearTarget(targetRoot, CFrame.new(arrowOffset), 0.28)
		elseif motionMode == "bridge" then
			local bridgeOffset = getBridgeOffset(companionIndex, companionCount)
			moveNearTarget(targetRoot, CFrame.new(bridgeOffset), 0.28)
		elseif motionMode == "dice" then
			local diceOffset = getDiceOffset(((companionIndex - 1) % 6) + 1)
			moveNearTarget(targetRoot, CFrame.new(diceOffset), 0.28)
...
subtitle.Text = "Commands: /follow /stack [name] /side line /orbit /line /fly /fireworks /stop /funfact /swarm [name] /cross /triangle /square /stairs /freeze /vline /zigzag /diamond /spiral /shield /gate /crown /arrow /bridge /dice /reset /say <msg> /calc <a+b|a/b> /auth <name> /unauth <name> /check"
...
local function processCommand(speaker, message)
	...
	elseif normalized == "/shield" then
		...
		return
	elseif normalized == "/gate" then
		startMotion("gate", speaker)
		return
	elseif normalized == "/crown" then
		startMotion("crown", speaker)
		return
	elseif normalized == "/arrow" then
		startMotion("arrow", speaker)
		return
	elseif normalized == "/bridge" then
		startMotion("bridge", speaker)
		return
	elseif normalized == "/dice" then
		startMotion("dice", speaker)
		return
...
local function startBotHeartbeat()
	task.defer(function()
		sendChatMessage(buildBotProtocolMessage("hello"))
	end)

	task.spawn(function()
		while true do
			task.wait(BOT_HEARTBEAT_INTERVAL)
			sendChatMessage(buildBotProtocolMessage("heartbeat"))
		end
	end)
end
