...
local BOT_HANDSHAKE_MESSAGE = "[bot-sync:v2]"
local BOT_HEARTBEAT_INTERVAL = 20
local BOT_STALE_TIMEOUT = 65
...
local function maybeRegisterBotFromMessage(player, message)
	if normalizeMessage(message) == normalizeMessage(BOT_HANDSHAKE_MESSAGE) then
...
local function startBotHeartbeat()
	task.defer(function()
		sendChatMessage(BOT_HANDSHAKE_MESSAGE)
	end)
