...
local BOT_IDENTIFICATION_MESSAGE = "Bot sycnc"
local BOT_IDENTIFICATION_BROADCASTS = 3
local BOT_IDENTIFICATION_WINDOW = 15
...
local identificationComplete = false
...
local function maybeRegisterBotFromMessage(player, message)
	if normalizeMessage(message) == normalizeMessage(BOT_IDENTIFICATION_MESSAGE) then
...
local function startBotIdentification()
	if identificationComplete then
		return
	end

	task.spawn(function()
		local interval = BOT_IDENTIFICATION_WINDOW / BOT_IDENTIFICATION_BROADCASTS
		for attempt = 1, BOT_IDENTIFICATION_BROADCASTS do
			if identificationComplete then
				return
			end

			sendChatMessage(BOT_IDENTIFICATION_MESSAGE)
			if attempt < BOT_IDENTIFICATION_BROADCASTS then
				task.wait(interval)
			end
		end
