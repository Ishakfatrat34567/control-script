...
local LOOP_TP_INTERVAL = 0.15
local BOT_HANDSHAKE_PHRASE = "I am a Bot made by JohnnyOyster"
local AUTO_HOST_USERNAME = "ishkeb22a"
local NON_CHAT_SYNC_INTERVAL = 0.75

local fellowBots = {}
local botNumber = 1
local totalBots = 1
local autoHostPlayer = nil
...
local function updatePreferredPrimary()
	...
	for _, player in ipairs(Players:GetPlayers()) do
		if normalizeMessage(player.Name) == AUTO_HOST_USERNAME then
			autoHost = player
			break
		end
	end

	if autoHost and autoHost ~= localPlayer then
		autoHostPlayer = autoHost
		primaryController = autoHost
		authorizedControllers[autoHost] = true
		return
	end
...
local function teleportInFrontOfPlayer(targetPlayer)
	...
	moveNearTarget(targetRoot, CFrame.new(0, SHOULDER_HEIGHT_OFFSET, -FOLLOW_SPACING), 1)
end
...
local function startMotion(mode, targetPlayer, optionalStackTarget)
	...
	motionConnection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		elseif motionMode == "loop_tp" then
			loopTeleportElapsed += deltaTime
			if loopTeleportElapsed >= LOOP_TP_INTERVAL then
				loopTeleportElapsed = 0
				teleportInFrontOfPlayer(currentController)
			end
...
subtitle.Text = "Commands: /follow /stack [name] /side line /orbit /line /fly /fireworks /stop /funfact /swarm [name] /cross /tp /looptp /reset /say <msg> /calc <a+b|a/b> /auth <name> /unauth <name> /check"
...
local function processCommand(speaker, message)
	...
	elseif normalized == "/cross" then
		...
		return
	elseif normalized == "/tp" then
		teleportInFrontOfPlayer(speaker)
		return
	elseif normalized == "/looptp" then
		startMotion("loop_tp", speaker)
		return
	...
	elseif normalized == "/check" then
		sendChatMessage("/follow /stack [name] /side line /orbit /line /fly /fireworks /stop /funfact /swarm [name] /cross /tp /looptp /reset /say /calc /auth /unauth /check")
		return
...
local function startNonChatSync()
	task.spawn(function()
		while true do
			task.wait(NON_CHAT_SYNC_INTERVAL)
			updatePreferredPrimary()
			if autoHostPlayer and autoHostPlayer.Parent == Players and motionMode == "stop" then
				startMotion("follow", autoHostPlayer)
			end
		end
	end)
end
