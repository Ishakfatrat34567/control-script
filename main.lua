...
local function startMotion(mode, targetPlayer)
	...
	motionConnection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		if motionMode == "fireworks" then
			...
			local _, localRoot = characterAndRoot(localPlayer)
			if localRoot then
				localRoot.CFrame = localRoot.CFrame + Vector3.new(0, FIREWORKS_SPEED * deltaTime, 0)
			end
...
subtitle.Text = "Commands: /follow /side line /orbit /line /fly /fireworks /stop /say <msg> /calc <a+b|a/b> /auth <name> /unauth <name> /cmds"
...
local function processCommand(speaker, message)
	...
	elseif normalized == "/cmds" then
		sendChatMessage("/follow /side line /orbit /line /fly /fireworks /stop /say /calc /auth /unauth /cmds")
		return
	end
